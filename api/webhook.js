...
function normalizePayload(body, query) {
  ...
  if (body.CallSid || body.CallTo) {
    return { processId, dialerSource:'exotel', callId:body.CallSid,
      agentNumber:body.To||body.CallTo, callerNumber:body.From||body.CallFrom,
      recordingUrl:body.RecordingUrl, duration:parseInt(body.RecordingDuration||0), status:body.Status };
  }
  if (body.ucid || body.campaign_id) {
    return { processId, dialerSource:'ozonetel', callId:body.ucid,
      agentNumber:body.agent_number, callerNumber:body.customer_number,
      recordingUrl:body.recording_url, duration:parseInt(body.duration||0), status:body.call_status };
  }
  return { processId, dialerSource:'generic',
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!payload.recordingUrl) return res.status(200).json({ ok:true, skipped:'no recording URL' });
    const db      = getDb();
    const callRef = await db.ref(`processes/${payload.processId}/calls`).push({
      ...payload, agentName:payload.agentNumber||null,
      status:'pending_transcription', transcript:null, aiSuggestions:null,
